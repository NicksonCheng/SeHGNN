...
def main(args):
    ...
    ## for heco dataset
    g, adjs, ratio_init_labels, num_classes, ratio_nid = load_dataset(args)
    for k in adjs.keys():
    ...
    ## heco dataset
        ...
        node_types = ["g", "d", "c", "s"]
        extra_metapath = []
    elif args.dataset == "HGB_Freebase_random_subg":
        tgt_type = "B"
        node_types = ["B", "F", "M", "S", "P", "L", "O"]
        extra_metapath = []
    elif args.dataset == "Freebase":
        tgt_type = "0"
    ...
    if args.dataset != "Freebase":
        ...
        if args.num_hops == 1:
            ...
            if os.path.exists(f"{save_name}_00_int64.npy"):
                ...
                raw_meta_adjs = hg_propagate_sparse_pyg(
                    adjs, tgt_type, args.num_hops, max_length, extra_metapath, prop_feats=True, echo=True, prop_device=prop_device
                )
    ...
    if args.dataset in ["DBLP", "ACM", "IMDB", "PubMed", "acm", "aminer", "freebase"]:
    ...
    for ratio in ratio_init_labels.keys():
        ...
        for i, seed in enumerate(args.seeds):
            ...
            checkpt_file = checkpt_folder + f"{seed}"
            ...
            train_nid = ratio_nid[ratio]["train"]
            val_nid = ratio_nid[ratio]["val"]
            test_nid = ratio_nid[ratio]["test"]
            ...
            if args.label_feats:
                ...
                if args.dataset == "Freebase":
                    ...
                    if args.dataset == "IMDB":
                        ...
                        condition = lambda ra, rb, rc, k: True
                        for k, v in label_feats.items():
                            print(f"keys:{k}, values:{v.shape}")
            ...
            # Train & eval loaders
            # =======
            train_loader = torch.utils.data.DataLoader(train_nid, batch_size=args.batch_size, shuffle=True, drop_last=False)
            ...
            for epoch in tqdm(range(args.epoch)):
                ...
                loss, acc = train(model, feats, label_feats, labels_cuda, loss_fcn, optimizer, train_loader, evaluator, num_classes, scalar=scalar)
                if not args.cpu:
                    torch.cuda.synchronize()
                end = time.time()
                ...
                log += f", Train ACC: {train_acc[2]:.4f}, Micro-F1: {train_acc[0]:.4f}, Macro-F1: {train_acc[1]:.4f})\n"
                log += f", Validation ACC: {val_acc[2]:.4f}, Micro-F1: {val_acc[0]:.4f}, Macro-F1: {val_acc[1]:.4f})\n"
                log += f", Test ACC: {test_acc[2]:.4f}, Micro-F1: {test_acc[0]:.4f}, Macro-F1: {test_acc[1]:.4f}) NMI: {test_acc[3]:.4f} ARI: {test_acc[4]:.4f}\n"
                ...
                if epoch > 0 and epoch % 10 == 0:
                    log = (
                        log
                        + f"\tCurrent best at epoch {best_epoch} with Val loss {best_val_loss:.4f} ({best_val[0]*100:.4f}, {best_val[1]*100:.4f})"
            ...
            print(
                f'Best Epoch {best_epoch} at {checkpt_file.split("/")[-1]}\n\tFinal Val loss {best_val_loss:.4f} ({best_val[0]*100:.4f}, {best_val[1]*100:.4f})'
                + f", Test loss {best_test_loss:.4f} ({best_test[0]*100:.4f}, {best_test[1]*100:.4f})"
            )

            total_best_test.append(best_test)
            ## clustering visualization
            if args.clus_visual and i == len(args.seeds) - 1:
            ...
            if args.dataset != "IMDB":
                ...
                dl.gen_file_for_evaluate(
                    test_idx=test_nid, label=pred, file_name=f"{args.dataset}_{args.seed}_{checkpt_file.split('/')[-1]}.txt", mode="multi"
                )
        ...
        if args.dataset != "PubMed" or args.dataset != "HGB_Freebase_random_subg":
            ...
            with open(f"log/{args.dataset}/{formatted_now}", "a") as log_file:
                log_file.write(f"Label Rate {ratio} Epoch: {epoch} Loss: {best_test_loss:.4f}\n")
                log_file.write(
                    f"ACC: {best_test[2]:.4f}, Micro-F1: {best_test[0]:.4f}, Macro-F1: {best_test[1]:.4f} NMI: {best_test[3]:.4f} ARI: {best_test[4]:.4f}\n"
                )
                log_file.close()
    ...
    if args.dataset == "PubMed" or args.dataset == "HGB_Freebase_random_subg":
        ...
        with open(f"log/{args.dataset}/{formatted_now}", "a") as log_file:
            log_file.write(f" Epoch: {epoch} Loss: {best_test_loss:.4f}\n")
            log_file.write(
                f"ACC: {best_test[2]:.4f}, Micro-F1: {best_test[0]:.4f}, Macro-F1: {best_test[1]:.4f} NMI: {best_test[3]:.4f} ARI: {best_test[4]:.4f}\n"
            )
            log_file.close()
...
def parse_args(args=None):
    ...
    parser.add_argument("--seeds", nargs="+", type=int, default=[1], help="the seed used in the training")
    parser.add_argument("--dataset", type=str, default="DBLP", choices=["DBLP", "ACM", "IMDB", "Freebase", "PubMed", "acm", "aminer", "freebase"])
    parser.add_argument("--gpu", type=int, default=1)
    parser.add_argument("--cpu", action="store_true", default=False)
    parser.add_argument("--root", type=str, default="../../data/")
    parser.add_argument("--epoch", type=int, default=50, help="Maxinum number of epochs.")
    parser.add_argument("--embed-size", type=int, default=256, help="inital embedding size of nodes with no attributes")
    parser.add_argument("--num-hops", type=int, default=2, help="number of hops for propagation of raw labels")
    parser.add_argument("--label-feats", action="store_true", default=False, help="whether to use the label propagated features")
    parser.add_argument("--clus_visual", default=False, help="draw cluster visualization or not")
    parser.add_argument("--num-label-hops", type=int, default=2, help="number of hops for propagation of raw features")
    ## For network structure
    parser.add_argument("--n-fp-layers", type=int, default=2, help="the number of mlp layers for feature projection")
    parser.add_argument("--n-task-layers", type=int, default=3, help="the number of mlp layers for the downstream task")
    ...
    parser.add_argument("--att-drop", type=float, default=0.0, help="attention dropout of model")
    parser.add_argument(
        "--act", type=str, default="none", choices=["none", "relu", "leaky_relu", "sigmoid"], help="the activation function of the transformer part"
    )
